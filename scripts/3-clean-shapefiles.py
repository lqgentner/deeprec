...
def clean_mrb(file_in: Path, file_out: Path):
    ...
    gdf = gdf.clean_names()
    gdf = gdf.rename(columns={"river_basi": "river"})
    # Removing parenthesis, UPPERCASE to Capitalized
    gdf.river = gdf.river.replace(" (.*)", "", regex=True).str.capitalize()
    # Sort after basin size
    ...
    gdf.river = gdf.river.where(
        ~gdf.river.duplicated(),
        gdf.river + "_" + gdf.groupby("river").cumcount().astype(str),
    )
