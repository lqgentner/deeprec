...
from deepwaters.regions import countries
from deepwaters.utils import month_center_range
...
def yearly2monthly(ds: XrObj) -> XrObj:
    """Reindexes a Dataset from a yearly to a monthly time dimension.
    The values are repeated in the forward direction,not interpolated.
    """
    time_yearly = ds.time.values
    if not pd.infer_freq(time_yearly) == "YS-JAN":
        raise ValueError("Dataset must have time dimension with freq='YS-JAN'.")
    ...
    return ds.reindex(time=time_monthly, method="ffill")
...
def align_time(
    ...
    time_month_start = pd.to_datetime(ds.time).floor("D") - pd.offsets.MonthBegin()
...
def chunk_dataset(ds: xr.Dataset) -> xr.Dataset:
    ...
    for var_name in list(ds.data_vars):
        ...
        if dims == {"time", "lat", "lon"}:
            ...
            elif len(ds.time) > 500:
                chunks = {"time": -1, "lat": 180, "lon": 180}
            else:
                chunks = {"time": -1, "lat": -1, "lon": 180}
        elif dims == {"time"}:
            chunks = {"time": -1}
        elif dims == {"lat", "lon"}:
            chunks = {"lat": -1, "lon": -1}
...
def combine_expver(da: XrObj) -> XrObj:
    """Combine ERA5T (early-release) and ERA5 data. Reduces
    the `expver` dimension."""
